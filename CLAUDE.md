...
視窗剛開起來的空窗期都可能漏一次）。三道護欄不能拿掉：
只核對 `entrypoint` 是 `claude-vscode` 的 session（終端機跑的本來就沒有分頁），
只核對「靜止」的列（狀態是 unknown/idle/done 且轉錄檔 5 分鐘沒動 —— 這同時決定了要不要花那 650ms 去掃，
全部都在跑的時候完全不掃），以及**一個編輯器視窗都沒掃到就不發布結果**
（空清單會讓每一列都判成「分頁不見了」，一口氣全部藏起來）。
掃描結果會寫進 `layout-diag.log`（`TABSCAN` / `TABMISS`），那是唯一看得到它有沒有在跑的地方。
代價是 Electron 的無障礙樹會一直開著（見下面 UIA 那段的副作用說明）。
